...
{
    public class AnalisisMacroCicloController : Controller
    {
        // GET: AnalisisMacroCiclo
        public ActionResult Index()
        {
            return View();
        }
        public ActionResult ListAnalisisMacroCiclo()
        {
            return View();
        }
        ...
        public struct ObjAnalisisMacroCiclo
        {
            public AnalisisMacrociclo AnalisisMacrocicloDepor { get; set; }
            public PlanAnalisisMacro PlanAnalisisMacroDepor { get; set; }
            public ObjetivosMacrociclo ObjetivosMacrocicloDepor { get; set; }
        ...
        [HttpGet]
        public JsonResult GetListAnalisisMacroCiclo()
        {
            ...
            {
                var DepoList = db.AnalisisMacrociclo.ToList().OrderByDescending(o => o.IdMacroAnterior);
             
                foreach (var item in DepoList)
                {
                    item.PlanAnalisisMacro = db.PlanAnalisisMacro.Where(w => w.IdMacroAnterior == item.IdMacroAnterior).ToList();
                    item.ObjetivosMacrociclo = db.ObjetivosMacrociclo.Where(w => w.IdMacroAnterior == item.IdMacroAnterior).ToList();
        ...
        [HttpGet]
        public JsonResult GetAnalisisMacroCicloById(int IdAnalisMacro)
        {
            Respuesta ret = new Respuesta();
            using (Models.BIOMEDICOEntities5 db = new Models.BIOMEDICOEntities5())

            {
                var MacroUpdate = db.AnalisisMacrociclo.FirstOrDefault(w => w.IdMacroAnterior == IdAnalisMacro);
                if (MacroUpdate != null)
                {
                    MacroUpdate.PlanAnalisisMacro = db.PlanAnalisisMacro.Where(w => w.IdMacroAnterior == MacroUpdate.IdMacroAnterior).ToList();
                    MacroUpdate.ObjetivosMacrociclo = db.ObjetivosMacrociclo.Where(w => w.IdMacroAnterior == MacroUpdate.IdMacroAnterior).ToList();
                }


                ret.objeto = MacroUpdate;
        ...
        public JsonResult Agregar(ObjAnalisisMacroCiclo a)
        {
            ...
            try
            {
...

                {
                    ...
                    a.AnalisisMacrocicloDepor.FechaRegistro = DateTime.Now;
                    a.AnalisisMacrocicloDepor.UsuarioRegistro = Utilidades.ActiveUser.NomUsuario;
                    
                    db.AnalisisMacrociclo.Add(a.AnalisisMacrocicloDepor);
                    db.SaveChanges();
                    var Id = a.AnalisisMacrocicloDepor.IdMacroAnterior;

                    //ADD ID TO TABLE DATAFAMILIA
                    a.PlanAnalisisMacroDepor.IdMacroAnterior = Id;
                    a.PlanAnalisisMacroDepor.AnalisisMacrociclo = db.AnalisisMacrociclo.FirstOrDefault(w => w.IdMacroAnterior == Id);

                    //add id to tabla ocupation

                    a.ObjetivosMacrocicloDepor.IdMacroAnterior = Id;
                    a.ObjetivosMacrocicloDepor.AnalisisMacrociclo = db.AnalisisMacrociclo.FirstOrDefault(w => w.IdMacroAnterior == Id);

                    db.PlanAnalisisMacro.Add(a.PlanAnalisisMacroDepor);
                    db.ObjetivosMacrociclo.Add(a.ObjetivosMacrocicloDepor);
                    db.SaveChanges();
