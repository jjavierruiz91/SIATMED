...
{
    public class PlanSemanalMicrocicloController : Controller
    {
        // GET: PlanSemanalMicrociclo
     ...
     public ActionResult ListaPlanSemanalMicrociclo()
        {
            return View();
        }
        ...
        public struct ObjPlanSemanalMicrociclo
        {
            public PlanSemanalMicrociclo PlanSemanalMicrocicloDeport { get; set; }

            public List<PlanSemanalDias> PlanSemanalDiasDeport { get; set; }
        ...
        [HttpGet]
        public JsonResult GetListPlanSemanalMicrociclo()
        {
            ...
            {
                var PlanSemanalMicrocicloExiste = db.PlanSemanalMicrociclo.ToList().OrderByDescending(o => o.IdPlanSemanal);

                foreach (var item in PlanSemanalMicrocicloExiste)
                {
                    item.PlanSemanalDias = db.PlanSemanalDias.Where(w => w.IdPlanSemanal == item.IdPlanSemanal).ToList();



                }
                ret.objeto = new { DepoListResul = PlanSemanalMicrocicloExiste };
        ...
        [HttpGet]
        public JsonResult GetPlanSemanalMicrocicloById(int IdPlanSemanal)
        {
            Respuesta ret = new Respuesta();
            using (Models.BIOMEDICOEntities5 db = new Models.BIOMEDICOEntities5())

            {
                var PlanSemanalMicrocicloUpdate = db.PlanSemanalMicrociclo.FirstOrDefault(w => w.IdPlanSemanal == IdPlanSemanal);
                if (PlanSemanalMicrocicloUpdate != null)
                {
                    PlanSemanalMicrocicloUpdate.PlanSemanalDias = db.PlanSemanalDias.Where(w => w.IdPlanSemanal == PlanSemanalMicrocicloUpdate.IdPlanSemanal).ToList();
                }

                ret.objeto = PlanSemanalMicrocicloUpdate;
        ...
        public JsonResult Agregar(ObjPlanSemanalMicrociclo a)
        {
            ...
            try
            {
...

                {
                    ...
                    a.PlanSemanalMicrocicloDeport.FechaRegistro = DateTime.Now;
                    a.PlanSemanalMicrocicloDeport.UsuarioRegistro = Utilidades.ActiveUser.NomUsuario;
                    ...
                    db.PlanSemanalMicrociclo.Add(a.PlanSemanalMicrocicloDeport);
                    db.SaveChanges();
                    var Id = a.PlanSemanalMicrocicloDeport.IdPlanSemanal;

                    a.PlanSemanalDiasDeport.ForEach(w => w.IdPlanSemanal = Id);
                    a.PlanSemanalDiasDeport.ForEach(s => s.PlanSemanalMicrociclo = db.PlanSemanalMicrociclo.FirstOrDefault(w => w.IdPlanSemanal == Id));

                    ////add id to tabla ocupation

                    db.PlanSemanalDias.AddRange(a.PlanSemanalDiasDeport);
                    db.SaveChanges();
