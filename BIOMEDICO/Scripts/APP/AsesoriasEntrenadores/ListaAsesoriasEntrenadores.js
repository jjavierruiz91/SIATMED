var TablaAsesoriasEntrenamiento = [];
let OpcionDeporte = "";
$(document).ready(function () {
    ...
    TablaAsesoriasEntrenamiento = $('#datatable-PlanEntrenamiento').DataTable();
    Get_Data(CargarTabla, '/AsesoriasEntrenadores/GetListAsesoriasEntrenadores')
...
function CargarTabla(data) {
    TablaAsesoriasEntrenamiento.clear().draw();
    let DataAsesoriasEntrenamiento = data.objeto.DepoListResul;
    if (DataAsesoriasEntrenamiento.length > 0) {
        DataAsesoriasEntrenamiento = DataAsesoriasEntrenamiento.filter(w => w.Deporte == OpcionDeporte);
    }
    console.log(DataAsesoriasEntrenamiento);
    $.each(DataAsesoriasEntrenamiento, function (index, item) {
        TablaAsesoriasEntrenamiento.row.add([
            item.NumIdentificacionDeportistas,
            item.Deportista,
            item.Entrenador,
            ...
            '<i class="btn btn-primary btn-group-sm fa fa-pencil-square-o" id="edit_ActEco_' + index + '" title="Modificar" style="fontsize:90px !important" onclick="ActualizarEntrenadoresData(' + item.IdAsesoriasEntrenadores + ')"></i>&ensp;' +
            '<i class="btn btn-info btn-group-sm icon-magazine" title="Detalle" onclick="DetalleData(' + item.IdAsesoriasEntrenadores + ')" ></i>&ensp;' +
...
function ActualizarEntrenadoresData(IdAsesoriasEntrenadoresDepor) {
    window.location.href = '../AsesoriasEntrenadores/Agregar?Id=' + IdAsesoriasEntrenadoresDepor;

}

function DetalleData(IdAsesoriasEntrenadoresDepor) {
    window.location.href = '../AsesoriasEntrenadores/Agregar?Id=' + IdAsesoriasEntrenadoresDepor + "&Viewdetail=SI";
...
function Eliminar(IdDepo) {
    swal({
        ...
        function (isConfirm) {
            if (isConfirm) {
                swal.close()
                Get_Data(RecargarTabla, '/AsesoriasEntrenadores/Eliminar?idDep=' + IdDepo);
            }
            else {
                swal.close()
            }
